...
function createAdvantage() {
	momentAdvantage = new PIXI.Container();
	momentGroup.addChild(momentAdvantage);
	
	var loader = new PIXI.loaders.Loader();
	loader.add("img/advantage/explosion.png");
	loader.add("img/advantage/shard1.png");
	loader.add("img/advantage/shard2.png");
	loader.add("img/advantage/shard3.png");
	...
	function onAssetsLoaded() {
		...
		//////////////// PARTICLE STUFF /////////////////
		advantageParticleContainer = new PIXI.Container();
		...
		for (var i = 0; i < 6; i++) {
			...
			if (matchVizViewConfig.isWebGL) advantageRectangle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) advantageShard1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) advantageShard2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) advantageShard3.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) advantageExplosion.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentAdvantage.addChild(advantageExplosion);
		
		// TEXT
		advantageTitle = new PIXI.Text("ADVANTAGE", { font: "80px Knockout47", fill: "#ffffff", padding: 15 });
		advantageTitle.position.x = 0;
		advantageTitle.position.y = 0;
		advantageTitle.anchor.x = 0.5;
		advantageTitle.anchor.y = 0.30;
...
function redrawAdvantage() {
	...
	for (var i = 0; i < 3; i++) {
		...
		if (matchVizViewConfig.isWebGL) advantagePlusArray[i].blendMode = PIXI.BLEND_MODES.MULTIPLY;
	}
	...
	if (matchVizViewConfig.isWebGL) advantageSquare1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
	...
	if (matchVizViewConfig.isWebGL) advantageSquare2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
	...
	if (matchVizViewConfig.isWebGL) advantageSquare3.blendMode = PIXI.BLEND_MODES.MULTIPLY;
...
function explodeAdvantage() {
	...
	advantageAnimationIn.add(TweenMax.fromTo( advantageTitle, 1,{ x: 0, y: 0, scaleX: 0, scaleY: 0 },{ x: 0, y: 0, scaleX: 1, scaleY: 1, ease: Elastic.easeOut }), 2);
	advantageAnimationIn.add(TweenMax.fromTo( advantageTitle, 1,{ alpha: 0 },{ alpha: 1, ease: Expo.easeOut }), 2);
